    ...
    with torch.no_grad():
        for batch_idx, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc="Generating prototypes"):
            ...
            if mask is not None:
                mask = mask.to(device)     
            B = features.size(0)
            batch_indices = torch.arange(total_samples_seen, total_samples_seen + B, device=device, dtype=torch.long)
            ...
            dataset_idx_exp = batch_indices.view(B, 1).expand(B, num_channels)  # (B, C)
...
class EpicTrainer(pl.LightningModule):
    def __init__(self, pointnet_model, num_channels=1024, lr: float = 1e-4, initial_topk=40, final_topk=5, max_epochs=20, point_subset_ratio: float = 1.0, subset_seed: int | None = None):
        super().__init__()
        ...
        for c in range(self.num_channels):
            ...
            for rank, sample_idx in enumerate(indices_for_c):
                ...
                if ply_path is None or not os.path.exists(ply_path):
                    ...
                    print(f"channel {c} ok found PLY for sample {sample_idx}")
